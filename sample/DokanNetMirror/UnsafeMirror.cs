...
internal class UnsafeMirror : Mirror, IDokanOperationsUnsafe
{
    ...
    public NtStatus ReadFile(ReadOnlySpan<char> fileName, IntPtr buffer, uint bufferLength, out int bytesRead, long offset, in DokanFileInfo info)
    {
        ...
        static void DoRead(FileStream stream, IntPtr innerBuffer, uint innerBufferLength, out int innerBytesRead, long innerOffset)
        {
    ...
    public NtStatus WriteFile(ReadOnlySpan<char> fileName, IntPtr buffer, uint bufferLength, out int bytesWritten, long offset, in DokanFileInfo info)
    {
        ...
        static void DoWrite(FileStream stream, IntPtr innerBuffer, uint innerBufferLength, out int innerBytesWritten, long innerOffset)
        {
            NativeMethods.SetFilePointer(stream.SafeFileHandle, innerOffset);
