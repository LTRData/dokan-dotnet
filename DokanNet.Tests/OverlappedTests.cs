...
public sealed class OverlappedTests
{
    ...
    [ClassInitialize]
    public static void ClassInitialize(TestContext _) => testData = DokanOperationsFixture.InitBlockTestData(FILE_BUFFER_SIZE, 5 * FILE_BUFFER_SIZE + 65536);

    [ClassCleanup]
    public static void ClassCleanup() => testData = null;

    [TestInitialize]
    public void Initialize() => DokanOperationsFixture.InitInstance(TestContext.TestName);

    [TestCleanup]
    public void Cleanup()
    {
        DokanOperationsFixture.ClearInstance(out var hasUnmatchedInvocations);
